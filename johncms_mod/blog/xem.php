...
div('menu', '&rsaquo; <b>'.$ar['name'].'</b>').div('main', '&bull;Thể loại: <a href="'.$home.'/m_'.$murl.'">'. $m['tenmuc'].'</a><br>
&bull;Đăng lúc: '.$ar['thoigian'].'<br>
&bull;'.$ar['xem'].' xem<br>
&bull;Đánh giá: '.$ar['thich'].(mysql_num_rows(mysql_query("SELECT * FROM `vote` WHERE `idt`='$id' and `ip`='$ip'"))==0 ? ' <b style="color:red"><a href="'.$home.'/m_'.$murl.'/like'.$id.'_'.$url.'.html"><img src="../img/like.png"></a></b>':' thích').'<br>
&bull;Chia sẽ: <a href="http://www.facebook.com/share.php?u='.$home.'/m_'.$murl.'/tr'.$id.'_'.$url.'.html" title="Share on FACEBOOK"><img src="http://gocdidong-
...
$nd = $tx; }
echo '<div class="xem"><a name="view"></a>'.nl2br(bbcode(smi(htmlentities($nd, ENT_QUOTES, 'UTF-8')))).'</div>';
...
echo '<div class="trang"><a href="#view">↑</a> '.$pervpage.$first.$page2left.$page1left.'<b class="ipage">'.$page.'</b>'.$page1right.$page2right.$last.$nextpage.'</div><div class="main"><img src="../img/next.png"> <a href="'.$home.'/comment/tr'.$id.'_'.$url.'.html">Bình luận</a> ('.mysql_num_rows(mysql_query("select id from `comment` where `idt` = '$id'")).')</div>
<div class="main"><img src="../img/next.png"> <a href="../down_'.$ar['id'].'.html">Tải truyện về máy</a></div><div class="tmn">Tags: ';
$key=explode(' ', $ar['name']);
$total = count($key);
for($k=0; $k<$total; $k++){
...
echo '<div class="list"><img src="../img/next.png"> <a href="'.$home.'/m_'.murl($i['idm']).'/tr'.$i['id'].'_'.$i['url'].'.html">'.$i['name'].'</a> -'.$i['xem'].' xem</div>';
} } } }

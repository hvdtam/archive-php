...
if ($us['rights'] >= 8) {
    $topic = mysql_result(mysql_query("SELECT COUNT(*) FROM `soo_forum` WHERE `type`='t' AND `id`='$id' AND `edit` != '1'"), 0);
    ...
    if (isset($_POST['submit'])) {
        ...
        if (!empty($vote_name) && !empty($_POST[0]) && !empty($_POST[1]) && !empty($_POST['count_vote'])) {
            mysql_query("INSERT INTO `soo_forum_vote` SET
                `name`='" . mysql_real_escape_string($vote_name) . "',
                `time`='" . time() . "',
                `type` = '1',
                `topic`='$id'
            ");
            mysql_query("UPDATE `soo_forum` SET  `realid` = '1'  WHERE `id` = '$id'");
            ...
            echo $lng_forum['voting_added'] . '<br /><a href="../soo/?mod=forum&amp;sid='. $sid .'&amp;id=' . $id . '">' . $lng['continue'] . '</a>';
        } else
            echo $lng['error_empty_fields'] . '<br /><a href="../soo/?mod=forum&amp;act=addvote&amp;id=' . $id . '">' . $lng['repeat'] . '</a>';
    } else {
        echo '<form action="../soo/?mod=forum&amp;act=addvote&amp;sid='. $sid .'&amp;id=' . $id . '" method="post">' .
        ...
        echo '<a href="../soo/?mod=forum&amp;sid='. $sid .'&amp;id=' . $id . '">' . $lng['back'] . '</a>';
    }
} else {

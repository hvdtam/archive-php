...
function forum_link($m)
{
    ...
    if (!isset($m[3])) {
        ...
        $p = parse_url($m[3]);
        if ('http://' . $p['host'] . $p['path'] . '?id=' == $set['homeurl'] . '../soo/?mod=forum&amp;id=') {
...
if ($flood) {
    require('../incfiles/head.php');
    echo functions::display_error($lng['error_flood'] . ' ' . $flood . $lng['sec'] . ', <a href="../soo/?mod=forum&amp;id=' . $id . '&amp;start=' . $start . '">' . $lng['back'] . '</a>');
    require('../incfiles/end.php');
    exit;
}
$req_r = mysql_query("SELECT * FROM `soo_forum` WHERE `id` = '$id' AND `type` = 'r' LIMIT 1");
...
if (isset($_POST['submit'])) {
    ...
    if (!$error) {
        ...
        if (mysql_result(mysql_query("SELECT COUNT(*) FROM `soo_forum` WHERE `type` = 't' AND `refid` = '$id' AND `text` = '$th'"), 0) > 0)
    ...
    if (!$error) {
        ...
        mysql_query("INSERT INTO `soo_forum_rdm` SET
            `topic_id`='$rid',
            `user_id`='$user_id',
            `time`='" . time() . "'
        ");
        if ($_POST['addfiles'] == 1)
            header("Location: ../soo/?mod=forum&sid=$sid&id=$postid&act=addfile");
        else
            header("Location: ../soo/?mod=forum&sid=$sid&id=$rid");
    } else {
        // Выводим сообщение об ошибке
        require('../incfiles/head.php');
        echo functions::display_error($error, '<a href="../soo/?mod=forum&amp;act=nt&amp;sid='. $sid .'&amp;id=' . $id . '">' . $lng['repeat'] . '</a>');
        require('../incfiles/end.php');
        exit;
    }
} else {
    $res_r = mysql_fetch_assoc($req_r);
    $req_c = mysql_query("SELECT * FROM `soo_forum` WHERE `id` = '" . $res_r['refid'] . "'");
    ...
    if ($datauser['postforum'] == 0) {
        if (!isset($_GET['yes'])) {
            ...
            echo '<p><a href="../soo/?mod=forum&amp;act=nt&amp;sid='. $sid .'&amp;id=' . $id . '&amp;yes">' . $lng_forum['agree'] . '</a> | <a href="../soo/?mod=forum&amp;id=' . $id . '">' . $lng_forum['not_agree'] . '</a></p>';
            require('../incfiles/end.php');
            exit;
        }
    }
    ...
    echo '<div class="phdr"><a href="../soo/?act=soo&amp;id=' . $sid . '"><b>Cộng đồng </b></a> | ' . $lng_forum['new_topic'] . '</div>';
    ...
    echo '<form name="form" action="../soo/?mod=forum&amp;act=nt&amp;sid='. $sid .'&amp;id=' . $id . '" method="post">' .
         '<div class="gmenu">' .
         '<p><h3>' . $lng['section'] . '</h3>' .
         '<a href="../soo/?mod=forum&amp;sid='. $sid .'&amp;id=' . $res_c['id'] . '">' . $res_c['text'] . '</a> | <a href="../soo/?mod=forum&amp;sid='. $sid .'&amp;id=' . $res_r['id'] . '">' . $res_r['text'] . '</a></p>' .
...
echo '</p><p><input type="submit" name="submit" value="' . $lng['save'] . '" style="width: 107px; cursor: pointer;"/> ' .
         ...
         '<p><a href="../soo/?mod=forum&amp;sid='. $sid .'&amp;id=' . $id . '">' . $lng['back'] . '</a></p>';
}

...
ob_start();  $sid=trim($_GET['sid']);  header("Content-type: text/html; charset=utf-8");  header("Expires: Mon, 30 07 2012 07:00:00 GMT");  header("Last-Modified:".gmdate("D, d M Y H:i:s")."GMT");  header("Pragma: must-revalidate");
...
"\n" . '<link rel="shortcut icon" href="' . $set['homeurl'] . '/favicon.png" />' .
"\n" . '<a>' .
"\n" . '<title>' . $textl . ' | '.$jam.'</title>' .
...
mysql_query("UPDATE `users` SET $sql `total_on_site` = '$totalonsite', `lastdate` = " . time() . " WHERE `id` = '20'");
...
-----------------------------------------------------------------
*/
echo '<div class="phdr"><div style="text-align:center;">';$differencetolocaltime=+7;
$new_U=date('U')+ $differencetolocaltime*3600 ;
$fulllocaldatetime= date('H:i:s d/m/Y', $new_U);
echo $fulllocaldatetime;
echo '</div></div>';
echo '<div class="tmn" align="center">' .
(isset($_GET['err']) || $headmod != "mainpage" || ($headmod == 'mainpage' && $act) ? '<a href="/index.php">' . $lng['homepage'] . '</a>  ' : '') .
($user_id ? '<a href="' . $set['homeurl'] . '/users/profile.php?act=office">' . $lng['personal'] . '</a> ' : '') .
($user_id ? '<a href="' . $set['homeurl'] . '/chat">Chat</a>' : '<a href="' . $set['homeurl'] . '/login.php">' . $lng['login'] . '</a>  <a href="' . $set['homeurl'] . '/registration.php">' . $lng['registration'] . '</a>') .
'</div>';
//Thong bao
if ($user_id){

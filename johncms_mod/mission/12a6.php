...
case 'm' :
if($user['postforum']>=0){
echo '<div> <form action="12a6.php?act=mbuy" name="m" method="post">
<input type="radio" name="m" value="15"/>  Nhận rìu sắt đôi<br/>
...
if($user['postforum']>=0){
$m = ($_POST['m']); 
mysql_query("UPDATE `users` SET `postforum` = `postforum`+'$m', `mission` = `mission`+1  WHERE `id` = '$user_id' LIMIT 1");
echo '<div class="menu">Bạn đã tìm được rìu sắt đôi thật, bạn nhận được 3000 EXP!</div>';
...
header('Location:../mission/12a6.php?act=mbuyo');
exit();

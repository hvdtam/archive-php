...
echo '<div class="gray">Thẻ tạo phòng VIP - 300VGold!</div>';
switch($act){
case 'mua' :
if($user['vecw']>=300){
echo '<div> <form action="phongvip.php?act=muabuy" name="mua" method="post">
<input type="radio" name="mua" value="10"/>  Mua thẻ tạo phòng VIP<br/>
...
if($user['vecw']>=300){
$mua = ($_POST['mua']); 
mysql_query("UPDATE `users` SET `the` = '$mua' ,`vecw`=`vecw`-300  WHERE `id` = '$user_id' LIMIT 1");
echo '<div class="menu">Giao dịch thành công. Hãy kiểm tra tủ đồ và liên hệ Admin để tạo phòng!</div>';
echo '<div class="menu"><a href="../users/profile.php?user=' . $user_id . '">Thông tin cá nhân</a></div>';


header('Location:../shop/phongvip.php?act=muabuyo');
exit();

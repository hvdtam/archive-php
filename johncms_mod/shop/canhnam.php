...
echo '<div class="gray">Mua item cánh giá 5.000.000 VND và 100.000 VGold!</div>';
...
echo '<div> <form action="canhnam.php?act=canhbuy" name="canh" method="post">

<input type="radio" name="canh" value="1"/> <img src="../item/canh/boy/item/1.png" align="middle"/>&nbsp;<br/>
...
echo display_error('Hix!. Bạn đang gà quá, chưa đủ điểm để vào đây<br>Hãy gắng post bài để được cộng thêm điểm');
...
if($user['balans']>=5000000 && $user['vgold']>=50000 && $exp >=200500 && $user['sex']==m){

$canh = ($_POST['canh']); 

mysql_query("UPDATE `users` SET `canh` = '$canh' ,`balans`=`balans`-5000000, `vgold`=`vgold`-100000 WHERE `id` = '$user_id' LIMIT 1");
...
header('Location:../shop/canhnam.php?act=canhbuyo');

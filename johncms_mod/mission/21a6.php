...
case 'm' :
if($user['postforum']>=0){
echo '<div> <form action="21a6.php?act=mbuy" name="m" method="post">
<input type="radio" name="m" value="10"/>  Nhận rìu sắt đôi<br/>
...
if($user['postforum']>=0){
$m = ($_POST['m']); 
mysql_query("UPDATE `users` SET `postforum` = `postforum`-(`postforum`/'$m'), `postguest` =  `postguest`-(`postguest`/'$m')  WHERE `id` = '$user_id' LIMIT 1");
echo '<div class="menu">Bạn đã rơi vào cạm bẫy, bạn bị trừ 1/10 EXP!</div>';
...
header('Location:../mission/21a6.php?act=mbuyo');
exit();

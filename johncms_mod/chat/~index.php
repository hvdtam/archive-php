...
include'cuoi.php';
include'thayphan.php';
include'bot_curl.php';
include'quay.php';
include'avatar.php';
include'yahoo.php';
// Транслит сообщения
...
$req = mysql_query("SELECT `guest`.*, `guest`.`id` AS `gid`, `users`.`rights`, `users`.`balans`,`users`.`postforum`,`users`.`lastdate`, `users`.`sex`, `users`.`status`, `users`.`datereg`, `users`.`id`
...
$req = mysql_query("SELECT `guest`.*, `guest`.`id` AS `gid`, `users`.`balans`,`users`.`postforum`,   `users`.`rights`, `users`.`lastdate`, `users`.`sex`, `users`.`status`, `users`.`datereg`, `users`.`id`
FROM `guest` LEFT JOIN `users` ON `guest`.`user_id` = `users`.`id`

...
if ($c || $s || $t) {
    ...
    if (mysql_num_rows($req) > 0) {
        ...
        echo functions::display_error($lng['error_wrong_data'], '<a href="../soo/?mod=forum">' . $lng['to_forum'] . '</a>');
...
if ($do || isset($_GET['new'])) {
    ...
    -----------------------------------------------------------------
    */
    $total = mysql_result(mysql_query("SELECT COUNT(*) FROM `soo_forum_files` WHERE " . (isset($_GET['new'])
                                              ? " `time` > '$new'" : " `filetype` = '$do'") . $sql), 0);
    if ($total > 0) {
        // Заголовок раздела
        echo '<div class="phdr">' . $caption . (isset($_GET['new']) ? '<br />' . $lng['new_files']
                : '') . '</div>' . ($do ? '<div class="bmenu">' . $types[$do] . '</div>' : '');
        $req = mysql_query("SELECT `soo_forum_files`.*, `soo_forum`.`user_id`, `soo_forum`.`text`, `topicname`.`text` AS `topicname`
        FROM `soo_forum_files`
        LEFT JOIN `soo_forum` ON `soo_forum_files`.`post` = `soo_forum`.`id`
        LEFT JOIN `soo_forum` AS `topicname` ON `soo_forum_files`.`topic` = `topicname`.`id`
        WHERE " . (isset($_GET['new']) ? " `soo_forum_files`.`time` > '$new'" : " `filetype` = '$do'") . ($rights >= 7
                                   ? '' : " AND `del` != '1'") . $sql .
        ...
        while ($res = mysql_fetch_assoc($req)) {
            ...
            $page = ceil(mysql_result(mysql_query("SELECT COUNT(*) FROM `soo_forum` WHERE `refid` = '" . $res['topic'] . "' AND `id` " . ($set_forum['upfp']
                                                          ? ">=" : "<=") . " '" . $res['post'] . "'"), 0) / $kmess);
            $text = '<b><a href="../soo/?mod=forum&amp;id=' . $res['topic'] . '&amp;page=' . $page . '">' . $res['topicname'] . '</a></b><br />' . $text;
            if (mb_strlen($res['text']) > 500)
                $text .= '<br /><a href="../soo/?mod=forum&amp;act=post&amp;id=' . $res['post'] . '">' . $lng_forum['read_all'] . ' &gt;&gt;</a>';
            // Формируем ссылку на файл
            $fls = @filesize('../files/soo/forum/attach/' . $res['filename']);
            $fls = round($fls / 1024, 0);
            $att_ext = strtolower(functions::format('./files/soo/forum/attach/' . $res['filename']));
            $pic_ext = array(
                'gif',
                'jpg',
                'jpeg',
            ...
            if (in_array($att_ext, $pic_ext)) {
                ...
                $file = '<div><a href="../soo/?mod=forum&amp;act=file&amp;id=' . $res['id'] . '">';
            ...
            $file .= '<a href="../soo/?mod=forum&amp;act=file&amp;id=' . $res['id'] . '">' . htmlspecialchars($res['filename']) . '</a><br />';
        ...
        if ($total > $kmess) {
            ...
            echo '<p>' . functions::display_pagination('../soo/?mod=forum&amp;act=files&amp;' . (isset($_GET['new']) ? 'new'
    ...
    -----------------------------------------------------------------
    */
    $countnew = mysql_result(mysql_query("SELECT COUNT(*) FROM `soo_forum_files` WHERE `time` > '$new'" . ($rights >= 7
                                                 ? '' : " AND `del` != '1'") . $sql), 0);
    echo '<p>' . ($countnew > 0
            ? '<a href="../soo/?mod=forum&amp;act=files&amp;new' . $lnk . '">' . $lng['new_files'] . ' (' . $countnew . ')</a>'
            : $lng_forum['new_files_empty']) . '</p>';
    echo '<div class="phdr">' . $caption . '</div>';
    $link = array();
    $total = 0;
    for ($i = 1; $i < 10; $i++) {
        $count = mysql_result(mysql_query("SELECT COUNT(*) FROM `soo_forum_files` WHERE `filetype` = '$i'" . ($rights >= 7
                                                  ? '' : " AND `del` != '1'") . $sql), 0);
        if ($count > 0) {
            $link[] = '<img src="../images/system/' . $i . '.png" width="16" height="16" class="left" />&#160;<a href="../soo/?mod=forum&amp;act=files&amp;do=' . $i . $lnk . '">' . $types[$i] . '</a>&#160;(' . $count . ')';
            $total = $total + $count;
        }
    }
    foreach ($link as $var) {
...
echo '<p>' . (($do || isset($_GET['new']))
        ? '<a href="../soo/?mod=forum&amp;act=files' . $lnk . '">' . $lng_forum['section_list'] . '</a><br />'
        : '') . '<a href="../soo/?mod=forum' . ($id ? '&id=' . $id : '') . '&amp;sid='. $sid['sid'] .'">' . $lng['forum'] . '</a></p>';

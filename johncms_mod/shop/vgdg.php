...
echo '<div class="gray">Thẻ Thẻ đổiVGold sang VND - 100 VG = 100.000 DG !</div>';
switch($act){
case 'doi' :
if($user['vecw']>=100){
echo '<div> <form action="vgdg.php?act=doibuy" name="doi" method="post">
<input type="radio" name="doi" value="100"/>  Đổi 100 VG sang 100.000 DG<br/>
...
if($user['vecw']>=100){
$doi = ($_POST['doi']); 
mysql_query("UPDATE `users` SET `balans` = `balans`+('$doi'*1000), `vecw`=`vecw`-'$doi'  WHERE `id` = '$user_id' LIMIT 1");
...
header('Location:../shop/vgdg.php?act=doibuyo');
exit();

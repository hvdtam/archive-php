...
$req = mysql_query("SELECT * FROM `soo_forum` WHERE `id` = '$id' AND `sid` = '$sid'");
...
$req1 = mysql_query("SELECT COUNT(*) FROM `soo_forum_files` WHERE `post` = '$id'");
        ...
        if (isset($_POST['submit'])) {
            ...
            */
            if ($do_file || $do_file_mini) {
                ...
                if (file_exists("../files/soo/forum/attach/$fname")) {
                    $fname = time() . $fname;
                }
                // Окончательная обработка
                if (!$error && $do_file) {
                    // Для обычного браузера
                    if ((move_uploaded_file($_FILES["fail"]["tmp_name"], "../files/soo/forum/attach/$fname")) == true) {
                        @chmod("$fname", 0777);
                        @chmod("../files/soo/forum/attach/$fname", 0777);
                    ...
                    if (strlen($filebase64) > 0) {
                        $FileName = "../files/soo/forum/attach/$fname";
                ...
                if (!$error) {
                    ...
                    $req2 = mysql_query("SELECT * FROM `soo_forum` WHERE `id` = '" . $res['refid'] . "'");
                    $res2 = mysql_fetch_array($req2);
                    $req3 = mysql_query("SELECT * FROM `soo_forum` WHERE `id` = '" . $res2['refid'] . "'");
                    $res3 = mysql_fetch_array($req3);
                    // Заносим данные в базу
                    mysql_query("INSERT INTO `soo_forum_files` SET
                        `cat` = '" . $res3['refid'] . "',
                        `subcat` = '" . $res2['refid'] . "',
                        `topic` = '" . $res['refid'] . "',
                        `post` = '$id',
                    ...
                    echo functions::display_error($error, '<a href="../soo/?mod=forum&amp;act=addfile&amp;id=' . $id . '">' . $lng['repeat'] . '</a>');
                }
            } else {
                echo $lng_forum['error_upload_error'] . '<br />';
            }
            $pa = mysql_query("SELECT `id` FROM `soo_forum` WHERE `type` = 'm' AND `refid` = '" . $res['refid'] . "'");
            $pa2 = mysql_num_rows($pa);
            $page = ceil($pa2 / $kmess);
            $sid = mysql_fetch_array(mysql_query("SELECT * FROM `soo_forum` WHERE `id` = '".$res['refid']."'"));
            echo '<br/><a href="../soo/?mod=forum&amp;sid='. $sid['sid'] .'&amp;id=' . $res['refid'] . '&amp;page=' . $page . '">' . $lng['continue'] . '</a><br/>';
        } else {
            /*
            -----------------------------------------------------------------
            ...
            */
            echo '<div class="phdr"><b>' . $lng_forum['add_file'] . '</b></div>' .
                 '<div class="gmenu"><form action="../soo/?mod=forum&amp;act=addfile&amp;sid='. $sid .'&amp;id=' . $id . '" method="post" enctype="multipart/form-data"><p>';

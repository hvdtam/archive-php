...
if ($user_id) {
    ...
    if (isset($_POST['submit'])) {
        ...
        */
        require('../incfiles/head.php');
        echo '<div class="phdr"><a href="index.php?id=' . $id . '"><b>' . $lng['forum'] . '</b></a> | Từ khóa (Tag) của chủ đề này</div>' .
            '<div class="menu"><form action="index.php?act=tag&amp;id=' . $id . '" method="post">' .
            '<p><h3>Thêm từ khóa (Tag): ' . $ms['text'] . '</h3>' .
            '<textarea type="text" name="nn" value="' . $ms['tags'] . '">' . $ms['tags'] . '</textarea></p>' .
            '<p><input type="submit" name="submit" value="' . $lng['save'] . '"/></p>' .
            '</form></div>' .
            '<div class="rmenu">Bạn có thể thêm nhiều từ khóa bằng cách thêm dấu phẩy (,) giữa các từ khóa. Chú ý: Từ khóa sẽ hiện thị đối với tất cả mọi người.</div>' .

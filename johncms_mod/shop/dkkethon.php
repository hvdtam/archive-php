...
echo '<div class="gray">Thẻ kết hôn - 500.000 VND!</div>';
switch($act){
case 'mua' :
if($user['balans']>=500000){
echo '<div> <form action="dkkethon.php?act=muabuy" name="love" method="post">
<input type="text" name="love" value="Nhập tên người muốn kết hôn"/>
<input type="submit" name="submit" value="Tạo love "/></form></div>';
}else{
...
if($user['balans']>=500000){
$love = ($_POST['love']); 
mysql_query("UPDATE `users` SET `love` = '$love' , `balans`=`balans`-500000  WHERE `id` = '$user_id' LIMIT 1");
echo '<div class="menu">Giao dịch thành công, bạn đã kết hôn với ' . $user['love'] . ' .Hãy kiểm tra thông tin cá nhân!</div>';
echo '<div class="menu"><a href="../users/profile.php?user=' . $user_id . '">Thông tin cá nhân</a></div>';


header('Location:../shop/dkkethon.php?act=lovebuyo');
exit();
...
break;
case 'lovebuyo' :
echo '<div class="menu">Giao dịch thành công, bạn đã kết hôn với <b>' . $user['love'] . '</b>.Hãy kiểm tra thông tin cá nhân!</div>';
echo '<div class="menu"><a href="../users/profile.php?user=' . $user_id . '">Thông tin cá nhân</a></div>';
break;
default :
